...
Repository* createRepository()
{
	...
	newRepository->profiles = createDynamicArray(DYNAMIC_ARRAY_CAPACITY, &destroyProfile);
...
void updateRepositoryNewDynamicArray(Repository* repository, DynamicArray* newDynamicArray)
{
	// destroy the old one
	destroyDynamicArray(repository->profiles);
	// make repository->profiles point to the new dynamicArray
	repository->profiles = newDynamicArray;
}
...
DynamicArray* getAllProfilesAsDynamicArray(Repository* repository)
{
...
Repository* getFilteredProfilesAfterYearsOfService(Repository* repository, int maximumYearsOfService)
{
	...
	for (int indexProfile = 0; indexProfile < numberProfiles; indexProfile++)
	{
		...
		{
			addElement(filteredProfilesRepository->profiles, copyProfile(profile)); //here copyProfile as the repo will
			//have to be destroyed and implicitly the profiles in it
		}
	}
